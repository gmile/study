...
RSpec::Matchers.define :be_folded do
  match do |example|
    ...
    @cyk.roots.map { |root| root.nterm.name }.include? expected_folding
  end

  failure_message_for_should do |example|
    message = "#{example.example_group.description.to_sym} expected, but "
    message << (@cyk.roots ? "[#{@cyk.roots.join(', ')}] found" : "it was unable to build tree")
  end
end
...
describe Cyk do
  ...
  context 'parameters' do
    ...
    it('(12)')                                      { example.should be_folded }
    it('(12+3, a)')                                 { example.should be_folded }
    it('(a, b, c)')                                 { example.should be_folded }
    it('(12+a+3, a)')                               { example.should be_folded }
    it("(a, b, 'string')")                          { example.should be_folded }
  end
